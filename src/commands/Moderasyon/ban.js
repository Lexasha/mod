...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    message.reply({ content: (`**${user}** - **(${user.id})** kullanıcısı ${author} tarafından **"${reason}"** sebebiyle sunucudan yasaklandı. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)}).catch((err) => console.log(err), client.ytick(message))
