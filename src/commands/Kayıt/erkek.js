...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!names) {
            message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı başarıyla kaydedildi`)] }).catch((err) => console.log(err), client.ytick(message))
        } else {
            message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı başarıyla kaydedildi.\n\n Kullanıcının toplamda " (${names.length}) " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`)] }).catch((err) => console.log(err), client.ytick(message))
        }

        client.channels.cache.get(config.channels.chat).send(`${member} aramıza hoş geldin.`).then((e) => setTimeout(() => { e.delete(); }, 5000));
    }
}