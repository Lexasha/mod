...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if(!member) { message.reply({ embeds: [embed.setDescription("Öncelikle geçerli bir kullanıcı belirtmelisin.")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        return }
        ...
        message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı başarıyla kayıtsıza <@&${config.registration.unregistered}> atıldı.`)] }).catch((err) => console.log(err), client.ytick(message))
